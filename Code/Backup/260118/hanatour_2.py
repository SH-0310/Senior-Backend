        ...
        with conn.cursor() as cursor:
            for p in products:
                ...
                for s in all_schedules:
                    dep_date = s.get('depDay')
                    sale_list = fetch_sale_products_by_day(rprs_code, dep_date, cookie)
                    for sp in sale_list:
                        booking_url = f"https://www.hanatour.com/trp/pkg/CHPC0PKG0200M200?pkgCd={sp['prefix']}{dep_date[2:]}{sp['suffix']}&prePage=major-products"
                        cursor.execute("""
                            INSERT INTO tour_schedules (product_code, departure_date, price_text, booking_url, updated_at)
                            VALUES (%s, %s, %s, %s, NOW()) 
                            ON DUPLICATE KEY UPDATE price_text=%s, booking_url=%s, updated_at=NOW(), error_msg=NULL
                        """, (rprs_code, dep_date, sp['price'], booking_url, sp['price'], booking_url))
