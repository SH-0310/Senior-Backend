...
def map_booking_status(state_sort, state, prefix):
    """예약 상태 판별"""
    if prefix == "출발확정" or state == 4 or state_sort == 0:
        return None  # ✅ '예약가능'은 NULL 처리
    elif state_sort == 1:
        return None  # ✅ '예약가능'은 NULL 처리
    elif state_sort == 2:
        return "대기예약"
    ...
    try:
        ...
        with conn.cursor() as cursor:
            for p in products:
                ...
                categories = classify_categories(master_title)
                cursor.execute("""
                ...
                """, (master_code, master_title, master_title, location, AGENCY_NAME, categories, MODE_PHONE, master_title, location, categories))
                stats["total_rprs"] += 1
                ...
                for detail in valid_details:
                    ...
                    status_text = map_booking_status(detail.get('stateSort'), detail.get('booking', {}).get('state'), detail.get('prefixPName'))
                    sdate = detail.get('date', {}).get('sdate', '').replace('-', '')
                    price = detail.get('price', {}).get('adult')
                    booking_url = f"https://www.modetour.com/package/{pnum}"
                    
                    cursor.execute("""
                        INSERT INTO tour_schedules (product_code, title, departure_date, price_text, booking_url, error_msg, updated_at)
                        VALUES (%s, %s, %s, %s, %s, %s, NOW())
                        ON DUPLICATE KEY UPDATE title=%s, price_text=%s, booking_url=%s, error_msg=%s, updated_at=NOW()
                    """, (master_code, child_title, sdate, price, booking_url, status_text, child_title, price, booking_url, status_text))
