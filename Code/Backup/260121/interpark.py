        ...
        with conn.cursor() as cursor:
            ...
            for idx, p_doc in enumerate(docs, 1):
                ...
                for c_doc in all_raw_docs:
                    ...
                    status = c_doc.get("bookingCode") or "예약가능"
                    ...
                    # 📝 [자식 로그] 예약 URL 포함 출력
                    logging.info(f"   ∟ 📅 {dep_date} | 💰 {price}원 | 🔗 {booking_url}")

                stats["saved_schedules"] += valid_count_for_this_tour
                if valid_count_for_this_tour == 0:
                    logging.info("   ∟ ⏳ (1개월 내 일정 없음)")
