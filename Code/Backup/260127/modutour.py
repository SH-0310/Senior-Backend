        ...
        with conn.cursor() as cursor:
            for p in products:
                ...
                if not valid_details:
                    continue 
                # ✅ 3. 부모 정보 변수 정의 (SQL 실행 전에 반드시 정의되어야 함)
                # p(상품 객체)에서 지역 정보를 가져와 location 변수에 저장
                location = p.get('areas', [{}])[0].get('name', '국내')
                # 제목을 분석하여 카테고리 분류 후 categories 변수에 저장
                categories = classify_categories(master_title)

                # ✅ 이 부분에 reference_code(추출한 asis_code)를 저장하도록 수정합니다.
                cursor.execute("""
                ...
                """, (
                    master_code, asis_code, master_title, master_title, location, AGENCY_NAME, categories, MODE_PHONE, 
                    asis_code, master_title, location, categories  # 👈 업데이트 시에도 반영
                ))
                stats["total_rprs"] += 1
                ...
                for detail in valid_details:
                    ...
                    # ✅ error_msg를 NULL로 넣어 검증기가 새로 수집된 데이터를 바로 인지하게 함
                    cursor.execute("""
                        ...
                        ON DUPLICATE KEY UPDATE 
                            title=%s, price_text=%s, booking_url=%s, error_msg=NULL, updated_at=NOW()
                    """, (master_code, child_title, sdate, price, booking_url, child_title, price, booking_url))
