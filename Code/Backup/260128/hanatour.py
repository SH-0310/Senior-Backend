        ...
        with conn.cursor() as cursor:
            for p in products:
                ...
                for s in all_schedules:
                    ...
                    # 🎯 [수정 핵심] 해당 날짜의 상세 상품(옵션 A, B 등) 리스트를 호출해야 합니다.
                    sale_list = fetch_sale_products_by_day(rprs_code, dep_date, cookie)
                    
                    if not sale_list: continue # 상세 데이터가 없으면 다음 날짜로
                    ...
                    for sp in sale_list:
                        ...
                        # ✅ tour_schedules의 title 컬럼에 sp['title'](상세 페이지 제목)을 저장
                        cursor.execute("""
                            ...
                            ON DUPLICATE KEY UPDATE 
                                ...
                                updated_at = NOW(), 
                                error_msg = NULL
                        """, (rprs_code, sp['title'], dep_date, sp['price'], booking_url, sp['title'], sp['price'], booking_url))
