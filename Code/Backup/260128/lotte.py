...
def get_child_schedules(cursor, god_id, m1, m2, m3, m4, parent_title, start_time):
    now = start_time
        ...
        try:
            ...
            for row in rows:
                ...
                """, (god_id, parent_title, formatted_date, price, 
                      booking_url, now, now, error_msg,
                      price, now, now, error_msg))
                saved_count += 1
...
def run_collection():
    ...
    try:
        with conn.cursor() as cursor:
            ...
            while True:
                ...
                for item in items:
                    ...
                    """, (god_id, god_id, title, description, location, start_time, AGENCY_NAME, categories, LOTTE_PHONE,
                          title, description, location, categories, start_time))
                    stats["total_rprs"] += 1

                    logging.info(f"📦 [{stats['total_rprs']}] {title[:20]}...")
                    child_count = get_child_schedules(cursor, god_id, m1, m2, m3, m4, title, start_time)
            ...
            # --- 이 부분의 들여쓰기가 수정되었습니다 (while 문 종료 후 실행) ---
        ...
        logging.info(f"🏁 {AGENCY_NAME} 수집 종료 (일정: {stats['saved_schedules']}건)")

    except Exception as e:
        logging.error(f"❌ 치명적 오류: {e}")
        # ✅ [추가] 에러 발생 시 FAIL 상태로 로그 기록
        try:
        ...
        send_telegram_msg(f"❌ {AGENCY_NAME} 수집 중 치명적 오류: {str(e)[:100]}")
    finally:
