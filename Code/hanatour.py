    ...
    try:
        ...
        with conn.cursor() as cursor:
            for p in products:
                ...
                cntnt_info = p.get('rprsCntntInfo', {})
                img_path = cntnt_info.get('rprsProdCntntUrlAdrs', "")
                
                if img_path:
                    # 실제 확인하신 최적화 경로(resize/400_0)를 포함하여 절대 경로 생성
                    main_img_url = f"https://image.hanatour.com/usr/cms/resize/400_0/{img_path}"
                else:
                    main_img_url = ""
                ...
                sql_tours = """
                    INSERT INTO tours (product_code, title, description, main_image_url, location, collected_at, agency, category, phone, is_priority)
                ...
                cursor.execute(sql_tours, (
                    rprs_code, title, title, main_img_url, location, AGENCY_NAME, categories, HANA_PHONE, 
                    title, main_img_url, location, categories
                ))
                stats["total_rprs"] += 1
